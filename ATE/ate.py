...
class TermExtractionProcessor(DataProcessor):
    ...
    def _create_examples(self, data, set_type):
        ...
        for i in range(len(data)):
            ...
            print(data['aspect_term_now'].loc[i])
            guid = "%s-%s" % (set_type, i)
            aspects=str(data['aspect_term_now'].loc[i]).split(",")
            if(len(aspects)==0 or aspects[0] == 'NULL' or aspects[0]=='[]'): # tagging: if no aspect term
                tags = ['O'] * s_length
            else:                                               # IOB format tag if aspect exist
                aspect_terms = [x.lower() for x in aspects] 
                tags = self.t_tagging_IOB(str(data['corrected_output'].loc[i]), aspect_terms)
...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # Evaluate the model (validation & test), print results.
        true_lbls, pred_lbls = evaluate(args, processor, model, tokenizer, device, "dev") #true, pred
        true_lbls_test, pred_lbls_test = evaluate(args, processor, model, tokenizer, device, "test")
